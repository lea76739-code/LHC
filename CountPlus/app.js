...
const STORAGE_KEY = "countplus-state-v8";
const MANUAL_GROUPS = [
...
function lineText(numbers, each, prefix, cumulative = null) {
  ...
  const numberText = numbers.map(twoDigit).join(separator);
...
function finalizePlan(groups, candidates, prefixes, limits = null) {
  ...
  const lines = groups.map((group) => {
    ...
    return {
      ...
      total,
      cumulative,
      text: lineText(group.numbers, group.each, randomItem(prefixes), cumulative),
    };
  });
...
function updateStats(plan = null) {
  ...
  if (plan) {
    ...
    const sum = amounts.reduce((acc, num) => acc + num, 0);
...
function ensureGenerationCandidates() {
  ...
  if (candidates.length || hasActiveSelectionFilter()) return candidates;
...
function refreshLineTotals(lines, prefixes) {
  ...
  lines.forEach((line) => {
    ...
    line.text = lineText(line.numbers, line.each, randomItem(prefixes), cumulative);
  });
  return lines;
}
...
function planText(plan) {
  return plan.lines.map((line) => line.text).join("\n");
}
...
function applyGeneratedPlan(plan, writeOutput = true) {
  ...
  if (!els.dataPanel.hidden) renderDataPanel(plan);
  saveState();
}
...
function scheduleAutoPreview() {
  ...
  autoPreviewTimer = setTimeout(() => {
    const candidates = activeCandidates();
    const plan = makeTierPlan(candidates, getCheckedValues("prefix")) || generatePlan(currentGroupCount());
    if (!plan) return;
...
function refreshAfterChange() {
  ...
  els.openDataBtn.textContent = "打开数据";
  els.resultOutput.value = "";
  setNotice("");
  saveState();
  const candidates = activeCandidates();
  if (candidates.length && hasBudgetAmount()) {
    const plan = makeTierPlan(candidates, getCheckedValues("prefix"));
    if (plan) {
      state.lastPlan = plan;
      updateStats(plan);
      setNotice(tierPlanNotice(plan));
...
function renderAllocationPanel(plan) {
  ...
  const title = plan.type === "tier" ? "三档分配" : "号码分配";
...
function bindEvents() {
  ...
  els.multiBtn.addEventListener("click", () => {
    const candidates = ensureGenerationCandidates();
    const cachedTierPlan =
      state.lastPlan?.type === "tier" && sameNumberList(state.lastPlan.candidates, candidates)
        ? state.lastPlan
        : null;
    const plan =
      hasBudgetAmount() && candidates.length
        ? cachedTierPlan || makeTierPlan(candidates, getCheckedValues("prefix"))
        : generatePlan(currentGroupCount());
    if (!plan) return;
    applyGeneratedPlan(plan, true);
  });

  els.openDataBtn.addEventListener("click", () => {
    if (!state.lastPlan) {
      const candidates = ensureGenerationCandidates();
      const cachedTierPlan =
        state.lastPlan?.type === "tier" && sameNumberList(state.lastPlan.candidates, candidates)
    ...
    if (els.dataPanel.hidden) {
      renderDataPanel(state.lastPlan);
      els.openDataBtn.textContent = "离开数据";
    } else {
      els.dataPanel.hidden = true;
      els.openDataBtn.textContent = "打开数据";
    }
